...
(function() {
    ...
    var WRIST_CENTER_OFFSET_DEG = 10.2;
    var WRIST_WRAP_REJECT_DEG = 90.0;
    var WRIST_SAFE_MIN_DEG = -75.0;
    var WRIST_SAFE_MAX_DEG = 75.0;
    var WRIST_FOLD_PITCH_ABS_DEG = 145.0;
    var WRIST_FOLD_LATCH_MIN_DEG = 30.0;
    var WRIST_FOLD_MARGIN_DEG = 3.0;
    var WRIST_FOLD_RELEASE_MARGIN_DEG = 1.0;
    var wristStabilizedDeg = null;
    var wristFoldLatched = false;
    var wristFoldSign = 0;
    var wristFoldHoldDeg = 0;
    ...
    function isWristFoldZone(handSensor) {
        return !!handSensor && Math.abs(handSensor.p) >= WRIST_FOLD_PITCH_ABS_DEG;
    }

    function stabilizeWristAngleDeg(rawDeg, handSensor) {
        var adjusted = normalizeHeadingDelta(rawDeg + WRIST_CENTER_OFFSET_DEG, 0);
        adjusted = Math.max(WRIST_SAFE_MIN_DEG, Math.min(WRIST_SAFE_MAX_DEG, adjusted));
        var foldZone = isWristFoldZone(handSensor);

        if (wristStabilizedDeg === null) {
            wristStabilizedDeg = adjusted;
            return adjusted;
        }

        var delta = normalizeHeadingDelta(adjusted, wristStabilizedDeg);
        if (Math.abs(delta) > WRIST_WRAP_REJECT_DEG) {
            adjusted = wristStabilizedDeg;
        }

        if (wristFoldLatched) {
            var backAcrossBoundary = false;
            if (wristFoldSign > 0) {
                backAcrossBoundary = adjusted < (wristFoldHoldDeg - WRIST_FOLD_RELEASE_MARGIN_DEG);
            } else if (wristFoldSign < 0) {
                backAcrossBoundary = adjusted > (wristFoldHoldDeg + WRIST_FOLD_RELEASE_MARGIN_DEG);
            }

            if (!foldZone && backAcrossBoundary) {
                wristFoldLatched = false;
                wristFoldSign = 0;
                wristFoldHoldDeg = 0;
            } else {
                wristStabilizedDeg = wristFoldHoldDeg;
                return wristStabilizedDeg;
            }
        }

        if (foldZone && Math.abs(wristStabilizedDeg) >= WRIST_FOLD_LATCH_MIN_DEG) {
            wristFoldSign = wristStabilizedDeg < 0 ? -1 : 1;
            wristFoldHoldDeg = Math.max(
                WRIST_SAFE_MIN_DEG,
                Math.min(WRIST_SAFE_MAX_DEG, wristStabilizedDeg + (wristFoldSign * WRIST_FOLD_MARGIN_DEG))
            );
            wristFoldLatched = true;
            wristStabilizedDeg = wristFoldHoldDeg;
            return wristStabilizedDeg;
        }

        delta = normalizeHeadingDelta(adjusted, wristStabilizedDeg);
        wristStabilizedDeg = Math.max(
            WRIST_SAFE_MIN_DEG,
            Math.min(WRIST_SAFE_MAX_DEG, wristStabilizedDeg + delta)
        );
        return wristStabilizedDeg;
    }
    ...
    function updateArmPose() {
        ...
        // Fuer den echten Wrist-Pfad ist die relative Roll-Differenz stabiler
        // als der Euler-basierte Quaternion-Twist, der am rechten Rand springen kann.
        var wristRawAngle = -normalizeHeadingDelta(imu.s[0].r, imu.s[1].r) * WRIST_RESPONSE_GAIN;
        var wristAngle = stabilizeWristAngleDeg(wristRawAngle, imu.s[0]);
