...
WRIST_CENTER_OFFSET_DEG = 10.2
WRIST_WRAP_REJECT_DEG = 90.0
WRIST_SAFE_MIN_DEG = -75.0
WRIST_SAFE_MAX_DEG = 75.0
WRIST_FOLD_PITCH_ABS_DEG = 145.0
WRIST_FOLD_LATCH_MIN_DEG = 30.0
WRIST_FOLD_MARGIN_DEG = 3.0
WRIST_FOLD_RELEASE_MARGIN_DEG = 1.0
...
def wrist_fold_zone_active(hand_pitch_deg):
    if hand_pitch_deg is None:
        return False
    return abs(float(hand_pitch_deg)) >= WRIST_FOLD_PITCH_ABS_DEG


def stabilize_wrist_angle(target_angle, previous_angle, hand_pitch_deg=None, fold_state=None):
    target_deg = clamp(rad2deg(target_angle), WRIST_SAFE_MIN_DEG, WRIST_SAFE_MAX_DEG)
    if previous_angle is None:
        return deg2rad(target_deg)

    previous_deg = rad2deg(previous_angle)
    delta_deg = normalize_heading_delta(target_deg, previous_deg)
    if abs(delta_deg) > WRIST_WRAP_REJECT_DEG:
        target_deg = previous_deg

    if fold_state is not None:
        fold_zone = wrist_fold_zone_active(hand_pitch_deg)

        if fold_state.get('latched', False):
            sign = fold_state.get('sign', 0.0)
            hold_deg = fold_state.get('hold_deg', previous_deg)
            back_across_boundary = False
            if sign > 0.0:
                back_across_boundary = target_deg < (hold_deg - WRIST_FOLD_RELEASE_MARGIN_DEG)
            elif sign < 0.0:
                back_across_boundary = target_deg > (hold_deg + WRIST_FOLD_RELEASE_MARGIN_DEG)

            if not fold_zone and back_across_boundary:
                fold_state['latched'] = False
                fold_state['sign'] = 0.0
                fold_state['hold_deg'] = 0.0
            else:
                return deg2rad(hold_deg)

        if fold_zone and abs(previous_deg) >= WRIST_FOLD_LATCH_MIN_DEG:
            sign = -1.0 if previous_deg < 0.0 else 1.0
            hold_deg = clamp(
                previous_deg + (sign * WRIST_FOLD_MARGIN_DEG),
                WRIST_SAFE_MIN_DEG,
                WRIST_SAFE_MAX_DEG,
            )
            fold_state['latched'] = True
            fold_state['sign'] = sign
            fold_state['hold_deg'] = hold_deg
            return deg2rad(hold_deg)

    delta_deg = normalize_heading_delta(target_deg, previous_deg)
    stabilized_deg = clamp(previous_deg + delta_deg, WRIST_SAFE_MIN_DEG, WRIST_SAFE_MAX_DEG)
    return deg2rad(stabilized_deg)
...
def relative_wrist_roll_deg(hand_sensor, forearm_sensor):
    return -normalize_heading_delta(hand_sensor['r'], forearm_sensor['r'])
...
def map_imu_payload(data, include_debug=False):
    ...
    wrist_roll_rel = relative_wrist_roll_deg(hand, forearm)
    wrist_angle_deg = normalize_heading_delta(
        wrist_roll_rel + WRIST_CENTER_OFFSET_DEG,
        0.0,
    ) * WRIST_RESPONSE_GAIN
    ...
    return joint_positions, {
        ...
        'reference': {
            ...
            'wrist_roll_offset': WRIST_CENTER_OFFSET_DEG,
        },
        'base': {
