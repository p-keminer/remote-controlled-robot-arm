...
from robotarm_description.mapping import map_imu_payload, stabilize_gripper_angle, stabilize_wrist_angle
...
class MqttBridge(Node):
    def __init__(self):
        ...
        self.wrist_fold_state = {'latched': False, 'sign': 0.0, 'hold_deg': 0.0}
    ...
    def publish_joint_state(self, data, payload_text=None):
        ...
        sensors = data.get('s', [])
        hand_pitch_deg = sensors[0].get('p') if len(sensors) > 0 else None
        joint_positions[3] = stabilize_wrist_angle(
            joint_positions[3],
            self.filtered_wrist_position,
            hand_pitch_deg=hand_pitch_deg,
            fold_state=self.wrist_fold_state,
        )
        self.filtered_wrist_position = joint_positions[3]
        ...
        if self.publish_debug and self.raw_pub is not None and self.mapping_pub is not None:
            ...
            debug_payload['wrist']['stabilized_angle'] = joint_positions[3]
            debug_payload['joint_positions']['wrist'] = joint_positions[3]
